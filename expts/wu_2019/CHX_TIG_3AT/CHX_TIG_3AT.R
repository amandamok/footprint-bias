...
sra_ids <- c("SRR7241918", "SRR8093857")
raw_data_dir <- file.path(here(), "expts", "wu_2019", "raw_data")

results_dir <- file.path(here(), "expts", "wu_2019", "CHX_TIG_3AT")

# 1. read in footprint alignments -----------------------------------------

bam_fname <- file.path(results_dir, "CHX_TIG_3AT.bam")
...
bam_dat_fname <- file.path(results_dir, "CHX_TIG_3AT_bam.Rda")
if(!file.exists(bam_dat_fname)) {
  CHX_TIG_3AT_bam <- load_bam(bam_fname, transcript_fa_fname, transcript_length_fname, offsets_fname,
                             f5_length=f5_length, f3_length=f3_length)
  save(CHX_TIG_3AT_bam, file=bam_dat_fname)
} else {
  load(bam_dat_fname)
}
...
subsets_fname <- file.path(results_dir, "CHX_TIG_3AT_subsets.Rda")
if(!file.exists(subsets_fname)) {
  d5_d3 <- count_d5_d3(CHX_TIG_3AT_bam)
...
transcript_counts <- aggregate(count ~ transcript, data=CHX_TIG_3AT_bam, FUN=sum)
...
training_fname <- file.path(results_dir, "CHX_TIG_3AT_training.Rda")
if(!file.exists(training_fname)) {
  CHX_TIG_3AT_training <- init_data(transcript_fa_fname, transcript_length_fname,
                                   d5_d3_subsets=d5_d3_subsets, f5_length=f5_length, f3_length=f3_length,
                                   which_transcripts=training_set)
  CHX_TIG_3AT_training$transcript <- relevel(CHX_TIG_3AT_training$transcript, ref=training_set[1])
  CHX_TIG_3AT_training$count <- count_footprints(CHX_TIG_3AT_bam, CHX_TIG_3AT_training, "count")
  save(CHX_TIG_3AT_training, file=training_fname)
} else {
  load(training_fname)
}
...
fit_fname <- file.path(results_dir, "CHX_TIG_3AT_fit_150.Rda")
if(!file.exists(fit_fname)) {
  CHX_TIG_3AT_fit_150 <- MASS::glm.nb(interxn_model, data=CHX_TIG_3AT_training, model=F)
  save(CHX_TIG_3AT_fit_150, file=fit_fname)
} else {
  load(fit_fname)
}
