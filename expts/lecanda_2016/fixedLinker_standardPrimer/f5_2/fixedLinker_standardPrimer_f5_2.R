...
results_dir <- file.path(here(), "expts", paper, expt, "f5_2")
...
load(file.path(here(), "expts", paper, expt, "correct_gc", paste0(bam_obj, ".Rda")))
assign(bam_obj,
       within(get(bam_obj),
              f5 <- substr(get(bam_obj)$f5, 1, 2)))
save(bam_obj, file=bam_fname)
