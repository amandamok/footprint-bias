...
regression_model <- formula(count ~ transcript + A + P + E + d5*genome_f5 + genome_f3 + gc)

paper <- "meydan_2020"
offsets_fname <- file.path(here(), "expts", paper, "Asite_rules_monosomes.txt")
...
assign(bam_obj,
       within(get(bam_obj),
              assign(paste0("correct_", num_genes),
                     correct_bias_nod3(get(bam_obj), get(fit_obj)))))
assign(bam_obj,
       within(get(bam_obj),
