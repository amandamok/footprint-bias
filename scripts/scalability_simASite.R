...
outputDir <- "~/footprint-bias/scalability_simASite"
refDir <- "~/simRiboSeq/refData"

source("~/simRiboSeq/scripts/helper.R")
source("~/footprint-bias/scripts/prep_data.R")

if(!("scalability_simASite" %in% list.files("~/footprint-bias"))) {
  system("mkdir ~/footprint-bias/scalability_simASite")
}

rawData_sam_fname <- "~/iXnos/expts/scer_biasDelta_withBias/process/scer_biasDelta_withBias_footprints.sam"
...
# rawData cts_by_codon data
rawData_cts_by_codon_fname <- "~/iXnos/expts/scer_biasDelta_withBias/process/cts_by_codon.size.27.31.txt"
rawData_cts_by_codon <- readRawProfiles(rawData_cts_by_codon_fname)
rawData_cts_by_transcript <- sapply(rawData_cts_by_codon, sum)
transcripts <- sort(rawData_cts_by_transcript, decreasing=T)
...
system(paste("grep -wFf", transcripts_fname, rawData_sam_fname, ">", sam_fname))
...
time_3_regression <- bench::mark(nb_fit_3 <- try(MASS::glm.nb(regression, data=regression_data_3)),
                                 iterations=1)
...
time_10_regression <- bench::mark(nb_fit_10 <- try(MASS::glm.nb(regression, data=regression_data_10)),
                                  iterations=1)
...
time_20_regression <- bench::mark(nb_fit_20 <- try(MASS::glm.nb(regression, data=regression_data_20)),
                                  iterations=1)
...
time_50_regression <- bench::mark(nb_fit_50 <- try(MASS::glm.nb(regression, data=regression_data_50)),
                                  iterations=1)
...
time_100_regression <- bench::mark(nb_fit_100 <- try(MASS::glm.nb(regression, data=regression_data_100)),
                                   iterations=1)
...
rmarkdown::render("scalability_simASite_eval.Rmd", output_format = "html_document")
