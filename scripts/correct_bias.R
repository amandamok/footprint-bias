...
correct_bias <- function(dat, fit, which_column="count") {
  ...
  # 2. establish f3 scaling factors
  f3_coefs <- coef(fit)[match(paste0("f3", fit$xlevels$f3), names(coef(fit)))]
  names(f3_coefs) <- fit$xlevels$f3
...
correct_bias_interxn <- function(dat, intrxn_fit, which_column="count") {
  ...
  f5_coefs <- expand.grid(f5=intrxn_fit$xlevels$f5, d5=intrxn_fit$xlevels$d5, stringsAsFactors=F)
  f5_coefs$correction <- sapply(seq(nrow(f5_coefs)),
                                   function(x) {
                                     ...
                                     if(tmp_f5 == f5_ref) {
                                       ...
                                       tmp_coef <- fit_coefs[paste0("f5", tmp_f5)]
                                     }
                                     if(tmp_d5 != d5_ref & tmp_f5 != f5_ref) {
                                       tmp_intrxn <- paste0("d5", tmp_d5, ":", "f5", tmp_f5)
  ...
  # 2. establish f3 scaling factors
  f3_ref <- intrxn_fit$xlevels$f3[1]
  d3_ref <- intrxn_fit$xlevels$d3[1]
  f3_coefs <- expand.grid(f3=intrxn_fit$xlevels$f3, d3=intrxn_fit$xlevels$d3, stringsAsFactors=F)
  f3_coefs$correction <- sapply(seq(nrow(f3_coefs)),
                                   function(x) {
                                     ...
                                     if(tmp_f3 == f3_ref) {
                                       ...
                                       tmp_coef <- fit_coefs[paste0("f3", tmp_f3)]
                                     }
                                     if(tmp_d3 != d3_ref & tmp_f3 != f3_ref) {
                                       tmp_intrxn <- paste0("d3", tmp_d3, ":", "f3", tmp_f3)
                                       if(!is.na(fit_coefs[tmp_intrxn])) {
                                         tmp_coef <- tmp_coef + fit_coefs[tmp_intrxn]
                                       }
                                     }
                      ...
                      lapply(seq_along(nb_fits),
                             function(x) {
                               ...
                               tmp_coefs <- data.frame(d5=subset_d5,
                                                       d3=subset_d3,
                                                       f5=nb_fits[[x]]$xlevels$f5,
                                                       stringsAsFactors=F)
                               tmp_coefs$correction <- subset_coefs[match(paste0("f5", tmp_coefs$f5),
                                                                          names(subset_coefs))]
  ...
  f3_coefs <- do.call(rbind,
                      lapply(seq_along(nb_fits),
                             function(x) {
                               ...
                               tmp_coefs <- data.frame(d5=subset_d5,
                                                       d3=subset_d3,
                                                       f3=nb_fits[[x]]$xlevels$f3,
                                                       stringsAsFactors=F)
                               tmp_coefs$correction <- subset_coefs[match(paste0("f3", tmp_coefs$f3),
                                                                          names(subset_coefs))]
                               tmp_coefs$correction[is.na(tmp_coefs$correction)] <- 0
                               tmp_coefs$correction <- exp(tmp_coefs$correction)
                               return(tmp_coefs)
