...
scalability_dir <- "~/footprint-bias/scalability/"
...
ggplot(memory, aes(x=nRows, y=mem)) + geom_line() + geom_text(aes(label=nGenes)) + facet_grid(~type) +
  theme_bw() + ylab("memory allocated (bytes)") + ggtitle("compute time") +
  theme(axis.text.x=element_text(angle=90))
...
all_coefs <- lapply(n,
                    function(x) {
                      tmp_coef <- coef(get(paste0("nb_fit_", x)))
                      nRows <- nrow(get(paste0("regression_data_", x)))
                      data.frame(value=tmp_coef,
                                 coef=names(tmp_coef),
                                 nGenes=x,
                                 nRows=nRows)
                    })
...
all_bias_plots <- lapply(n,
                         function(x) {
                           ...
                           tmp_transcripts_fa_fname <- file.path("~/footprint-bias/scalability", 
                                                                 paste0("genes_", x), 
...
do.call(wrap_plots, list(all_bias_plots, ncol=1))
```